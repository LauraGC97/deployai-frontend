...
export class HomeComponent implements AfterViewInit {
  ...
  private initBackground(): void {
    ...
    const streams = Array.from({ length: 30 }, () => ({
      ...
      // ── SUBIDA DE OPACIDAD: antes 0.05-0.12, ahora 0.12-0.25 ──
      opacity: 0.12 + Math.random() * 0.13,
    }));

    const waves = [
      // ── ALPHA DOBLADO en todas las ondas para que se vean claramente ──
      { y: 0.25, amp: 22, freq: 0.007, speed: 0.004, color: [34,211,238],  alpha: 0.13 },
      { y: 0.50, amp: 30, freq: 0.005, speed: 0.003, color: [168,85,247],  alpha: 0.11 },
      { y: 0.72, amp: 18, freq: 0.010, speed: 0.005, color: [16,185,129],  alpha: 0.09 },
      { y: 0.88, amp: 24, freq: 0.006, speed: 0.002, color: [253,224,71],  alpha: 0.07 },
    ];
    ...
    const draw = () => {
      ...
      waves.forEach(w => {
        ...
        // Brillo bajo la ola — también más visible
        const grad = ctx.createLinearGradient(0, cy - w.amp, 0, cy + w.amp * 3);
        grad.addColorStop(0, `rgba(${r},${g},${b},${w.alpha * 0.5})`);
      ...
      streams.forEach(s => {
        ...
        s.chars.forEach((ch, i) => {
          ...
          // ── Primer char más brillante: cap subido de 0.3 → 0.55 ──
          const bright = i === 0 ? Math.min(alpha * 3, 0.55) : alpha;
